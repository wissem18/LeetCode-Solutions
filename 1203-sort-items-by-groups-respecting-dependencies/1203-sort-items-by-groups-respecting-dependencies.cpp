class Solution {
public:
    vector<int> topological_sort( vector<vector<int>>& adj, vector<int>& deg, int sz){
    queue<int> q;
    for (int i=0; i<sz; i++)
        if (deg[i]==0) q.push(i);
    vector<int> res;
...
vector<int> sortItems(int n, int m, vector<int>& group, vector<vector<int>>& beforeItems) {
    ...
    vector<int> sort_group=topological_sort(adj_group, deg_group, m);
    vector<int> sort_item=topological_sort(adj_item, deg_item, n);
